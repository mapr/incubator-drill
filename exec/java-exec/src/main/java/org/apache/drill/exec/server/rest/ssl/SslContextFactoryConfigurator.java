...
import org.apache.drill.exec.ssl.SSLConfigServer;
...
public class SslContextFactoryConfigurator {
  ...
  /**
   * Tries to apply yarn basic ssl options configured by user. If provided configuration isn't valid,
   * new self-signed certificate will be generated and used in sslContextFactory.
   *
   * @param  configMap
   *           The specific drillOnYarn ssl parameters, need for creating @SSLConfigServer
   * @return new configured sslContextFactory
   * @throws Exception when generation of self-signed certificate failed
   */
  public SslContextFactory configureNewDOYContextFactory(HashMap<String,String> configMap) throws Exception {
    ...
    SslContextFactory.Server sslContextFactory = new SslContextFactory.Server();
    if (sslConf.isSslValid()) {
      setBasicOptions(sslContextFactory, sslConf);
    } else {
      useAutoGeneratedSelfSignedCertificate(sslContextFactory);
    }
    return sslContextFactory;
  ...
  public void setBasicOptions(SslContextFactory.Server sslFactory, SSLConfig sslConf){
  ...
  private void useOptionsConfiguredByUser(SslContextFactory.Server sslFactory, SSLConfig sslConf) {
    ...
    setBasicOptions(sslFactory, sslConf);
